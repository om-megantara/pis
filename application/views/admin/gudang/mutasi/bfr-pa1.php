...
<form id="demoform5p" action="?p=mut-gdgpa1-proses" method="post">
    ...
    <div class="col-sm-8">
        <select multiple="multiple" size="10" name="duallistbox_demo2[]" id="duallistbox_demo2[]">
            ...
            //$tampil=mysql_query("select * from gudang_paku where status_gudang='2.3' And status_barang = 'Baru' order by kode");
            // $link = mysqli_connect("localhost:3307", "root", "", "produksi");
            $tampil = mysqlI_query($link, "select * from gudang_paku where status_gudang='2.3' And status_barang != 'Habis' order by kode");
            $no = 1;
            $jumtot = 0;
            while ($row = mysqlI_fetch_array($tampil, MYSQLI_ASSOC)) {
            ?>
                <tr>
                    <option value="<?php echo $row['kode']; ?>"><?php echo $row['diameter'] . " | " . $row['kode_barang'] . " | " . $row['berat']; ?></option>
                    <td></td>
                </tr>
            <?php
                $no++;
