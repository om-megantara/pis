...
<?php if (isset($_POST["btn_proses"]) == "") { ?>
    <div class="page-content">
        ...
        <div class="col-lg-12">
            ...
            <form method="POST" action="<?= base_url('rekap/hasil-kd'); ?>">
                <div class="row"></div>
                <div id="page-wrapper">
    ...
    <div class="page-content">
        ...
        <div class="col-lg-12">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <h2>Laporan Rekapitulasi Produksi Kawat Duri: <?php echo $_POST["txttgl_aw"] . " sampai " . $_POST["txttgl_ak"]; ?></h2>
                        <div id="print-area-1" class="print-area">
                            <div class="row">
                                <table class="tables table-hover" border="1" rules="all">
                                    ...
                                    // $link = mysqli_connect("localhost:3307", "root", "", "produksi");
                                    $r_tgl1 = mysqli_query($link, "SELECT * FROM kawat_duri where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak'" . $syarat_tim . " AND status_del='0' group by tanggal order by tanggal asc") or die(mysqli_error($link));
							...
							$r_tgl1=mysqli_query($link, "SELECT * FROM galvanis where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak'  AND status_del='0' group by tanggal order by tanggal") or die(mysqli_error($link));
                                    ...
                                    while ($r_tgl2 = mysqli_fetch_array($r_tgl1)) {
                                        ...
                                        $r_tgl = mysqli_query($link, "SELECT * FROM kawat_duri where tanggal like '$tanggal2'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal,ukuran desc") or die(mysqli_error($link));
										...
										$target=750;//per jam
                                            ...
                                            $tanggal = $row["tanggal"];
                                            $baik_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_roll) as 'hasil_baik_rol' FROM kawat_duri where tanggal like '$tanggal' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $baik_rol = $baik_rol['hasil_baik_rol'];

                                            $baik_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_berat) as 'hasil_baik_berat' FROM kawat_duri where tanggal like '$tanggal' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $baik_berat = $baik_kg['hasil_baik_berat'];

                                            $sb_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_roll) as 'hasil_sb_roll' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $sb_rol = $sb_rol['hasil_sb_roll'];
                                            $sb_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_berat) as 'hasil_sb_berat' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $sb_berat = $sb_kg['hasil_sb_berat'];

                                            $tot_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_rol) as 'total_roll' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $total_rol = $tot_rol['total_roll'];

                                            $tot_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_berat) as 'total_berat' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $total_berat = $tot_kg['total_berat'];

                                            $aval_b = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_baik) as 'aval_baik' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $aval_baik = $aval_b['aval_baik'];
                                            $aval_r = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_ruwet) as 'aval_ruwet' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            $aval_ruwet = $aval_r['aval_ruwet'];

                                            $jam = mysqli_fetch_array(mysqli_query($link, "SELECT sum(jam_kerja) as 'jam_kerja' FROM kawat_duri where tanggal like '$tanggal2' and ukuran='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by ukuran order by tanggal"));
                                            ...
                                            <tbody>
                                                <tr class="tengah">
                                                    <td style="font-size:11px;"><?php echo $tanggal;
                                                                                $no ?></td>
                                                    <td style="font-size:11px;"><?php echo $ukuran; ?></td>
