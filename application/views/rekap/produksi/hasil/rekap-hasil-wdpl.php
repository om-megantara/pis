...
<?php if (isset($_POST["btn_proses"]) == "") { ?>
    <div class="page-content">
        ...
        <div class="col-lg-12">
            ...
            <form method="POST" action="<?= base_url('rekap/hasil-wdpl'); ?>">
                <div class="row"></div>
                <div id="page-wrapper">
        ...
        <div class="col-lg-12">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <h2>
                            <center>Laporan Rekapitulasi Produksi WD PL: <?php echo $_POST["txttgl_aw"] . " sampai " . $_POST["txttgl_ak"] . "<br>(" . $tim . ")"; ?></center>
                        </h2>
							...
							$r_tgl1=mysqli_query($link, "SELECT * FROM galvanis where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak'  AND status_del='0' group by tanggal order by tanggal") or die(mysqli_error($link));
                                    ...
                                    while ($r_tgl2 = mysqli_fetch_array($r_tgl1)) {
										...
										$target=750;//per jam
                                            ...
                                            $tanggal = $row["tanggal"];
                                            $baik_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_roll) as 'hasil_baik_rol' FROM wdpl where tanggal like '$tanggal' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $baik_rol = $baik_rol['hasil_baik_rol'];

                                            $baik_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_berat) as 'hasil_baik_berat' FROM wdpl where tanggal like '$tanggal' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $baik_berat = $baik_kg['hasil_baik_berat'];

                                            $sb_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_roll) as 'hasil_sb_roll' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $sb_rol = $sb_rol['hasil_sb_roll'];
                                            $sb_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_berat) as 'hasil_sb_berat' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $sb_berat = $sb_kg['hasil_sb_berat'];

                                            $tot_rol = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_rol) as 'total_roll' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $total_rol = $tot_rol['total_roll'];

                                            $tot_kg = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_berat) as 'total_berat' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $total_berat = $tot_kg['total_berat'];

                                            $aval_b = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_baik) as 'aval_baik' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $aval_baik = $aval_b['aval_baik'];
                                            $aval_r = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_ruwet) as 'aval_ruwet' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                            $aval_ruwet = $aval_r['aval_ruwet'];

                                            $jam = mysqli_fetch_array(mysqli_query($link, "SELECT sum(jam_kerja) as 'jam_kerja' FROM wdpl where tanggal like '$tanggal2' and diameter='$r_ukuran'" . $syarat_tim . " AND status_del='0' group by diameter order by tanggal"));
                                    ...
                                    ?>
                                        ...
                                        //$brol=$brol+$baik_roll;
                                            ...
                                            <tr>
                                                ...
                                                <td style="font-size:12px;color:blue" class="tengah"><b><?php echo number_format((float) $tkg, 2, '.', ','); ?></b></td>
                                        ...
                                        <div class="col-lg-6">
                                            <div class="container-fluid">
                                                ...
                                                <table class="tables table-hover" border="1" rules="all">
                                                    ...
                                                    <?php
                                                    $dia = mysqli_query($link, "SELECT * FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak'" . $syarat_tim . " AND status_del='0' group by diameter order by diameter asc") or die(mysqli_error($link));
                                                    while ($rowd = mysqli_fetch_array($dia)) {
                                                        $diam = $rowd['diameter'];

                                                        $rolb = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_roll) as 'hasil_baik_roll' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_rolb = $rolb['hasil_baik_roll'];

                                                        $rols = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_roll) as 'hasil_sb_roll' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_rols = $rols['hasil_sb_roll'];

                                                        $kgb = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_baik_berat) as 'hasil_baik_berat' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_kgb = $kgb['hasil_baik_berat'];

                                                        $kgs = mysqli_fetch_array(mysqli_query($link, "SELECT sum(hasil_sb_berat) as 'hasil_sb_berat' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_kgs = $kgs['hasil_sb_berat'];

                                                        $totr = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_rol) as 'total_roll' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_trol = $totr['total_roll'];

                                                        $totb = mysqli_fetch_array(mysqli_query($link, "SELECT sum(total_berat) as 'total_berat' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $totald_tkg = $totb['total_berat'];

                                                        $taval_b = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_baik) as 'aval_baik' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
                                                        $taval_b = $taval_b['aval_baik'];

                                                        $taval_s = mysqli_fetch_array(mysqli_query($link, "SELECT sum(aval_ruwet) as 'aval_ruwet' FROM wdpl where tanggal>='$tgl_aw' AND tanggal<='$tgl_ak' AND diameter='$diam'" . $syarat_tim . "  AND status_del='0'"));
